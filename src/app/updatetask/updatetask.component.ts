...
export class UpdatetaskComponent implements OnInit {
  ...
  constructor(private _task : TaskserviceService, private route : ActivatedRoute) {
    this.item = new Task(); 
  ...
  {
    return ((this.item.TaskDesc != null) && (this.newendDate != '') 
    && (this.newstartDate != '') && (this.item.priority != null) && 
  ...
  getTaskinfo(task_id : number)
  {
      ...
      this._def.forEach(element => {
        this._task.getParentTasksByID(element.Parent_Id ).subscribe((parent) => {           
          console.log(parent[0].Parent_Task);
